...
import com.demo.recommend.client.RedisClient;
...
import com.demo.recommend.service.ContactService;
import com.demo.recommend.service.ProductService;
import com.demo.recommend.service.RecommendService;
import com.demo.recommend.service.UserScoreService;
...
public class RecommendServiceImpl implements RecommendService {
    ...
    @Override
    public List<ProductScoreEntity> userRecommend(String userId) throws IOException {
        ...
        randProduct.forEach(r -> {
            try {
                rst.add(r);
                List<Map.Entry<String, Double>> ps = HbaseClient.getRow("ps", userId);
                int end = Math.max(ps.size(), 3);
                for (int i = 0; i < end; i++) {
                    Map.Entry<String, Double> entry = ps.get(i);
                    ProductEntity p = productService.selectById(entry.getKey());
    ...
    @Override
    public List<ProductDto> recommendByItemCfCoeff() {
        List<String> topList = getDefaultTop();
    ...
    @Override
    public List<ProductDto> recommendByProductCoeff() {
        List<String> topList = getDefaultTop();
    ...
    private List<String> addRecommendProduct(List<String> topList, String table) {
        ...
        for (String s : topList) {
            //首先将top产品添加进结果集
            ret.add(s);
            List<Map.Entry<String, Double>> ps = new ArrayList<>();
